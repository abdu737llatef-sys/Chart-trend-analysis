...
if('serviceWorker' in navigator)window.addEventListener('load',async()=>{try{
  const reg=await navigator.serviceWorker.register('./service-worker.js?v=5.6.7.2',{updateViaCache:'none'});
  await reg.update();
...
window.openFull=symbol=>{location.href=`./?v=5.6.7.2&symbol=${encodeURIComponent(symbol)}`;};
...
$('runScannerBtn').onclick=async()=>{
 ...
 try{
   ...
   $('scannerSummary').innerHTML=`<h2>V5.6.7.2 Scanner Summary</h2><div class="metrics">
