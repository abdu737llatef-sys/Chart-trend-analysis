...
if('serviceWorker' in navigator)window.addEventListener('load',async()=>{try{
  const reg=await navigator.serviceWorker.register('./service-worker.js?v=5.6.7.7',{updateViaCache:'none'});
  await reg.update();
...
window.openFull=symbol=>{const m=$('scanMarket')?.value||'futures';location.href=`./?v=5.6.7.7&symbol=${encodeURIComponent(symbol)}&market=${encodeURIComponent(m)}`;};
...
$('runScannerBtn').onclick=async()=>{
 ...
 try{
   ...
   $('scannerSummary').innerHTML=`<h2>V5.6.7.7 Unified Scanner Summary</h2><div class="metrics">
   ...
   $('status').textContent=`Stage 4/4 complete: ${bulls.length} Bullish و${bears.length} Bearish اجتازوا Directional + Execution + Integrity gates في V5.6.7.7.`;
 }catch(e){
